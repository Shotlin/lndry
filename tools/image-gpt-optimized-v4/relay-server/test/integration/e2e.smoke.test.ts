/**
 * End-to-end smoke test — task 21.4 (per integration contract).
 *
 * Boots a real `relay-server` in-process by calling `httpServer.listen(0)`
 * through the workspace-root harness, then connects:
 *
 *   - a mock browser-agent driven by real `socket.io-client`,
 *   - a real {@link ExtensionRelayClient} from the `kiro-extension/` package.
 *
 * The mock agent receives a single chat dispatch, streams a fixed
 * sequence of partial chunks, and emits a final chunk whose `text`
 * field is the full concatenation of the partials. The test asserts
 * that the extension's observed final chunk's `text` is byte-equal to
 * that concatenation.
 *
 ...
 *  - Chunk routing to the originating client (R7.4 fan-out) — every
 ...
 *
 * The harness's `shutdown()` calls `server.shutdown(0)` followed by
 * `httpServer.close()` for deterministic tear-down.
 *
 * _Implements: R5.1, R5.5, R7.4, R7.6_
 */
...
import {
  ...
  HARNESS_KIRO_SECRET,
  type RelayHarness,
} from '../../../test/e2e/harness.js';

/**
 * Wait helper — returns a promise that resolves once `predicate()` is
 * truthy or `timeoutMs` elapses. Polls every 10 ms which is plenty
 * fast for socket round-trips on localhost.
 */
...
/**
 * Minimal mock browser-agent built on real `socket.io-client`.
 *
 * Connects to the relay with the agent secret, captures the
 * server-issued `agentId`, listens for `agent.dispatch`, streams a
 * sequence of partial chunks, and finishes with one final chunk whose
 * `text` is the full assembled string.
 */
interface MockAgent {
  ...
  /** Number of chunks emitted by this agent during the test run. */
 ...
 * dispatched request:
 *   1. Send `agent.ack` to clear the relay's ack timer.
 *   2. Emit `partials.length` partial chunks.
 *   3. Emit one final chunk whose `text` is the full assembled
 *      string, carrying `isFinal: true` and `status: 'completed'`.
 */
function createMockAgent(opts: {
  ...
  // sends one immediately on connect so `lastHeartbeatAt` is updated;
  // the relay's 45 s heartbeat-miss window is well past any
...
describe('e2e smoke (task 21.4) — relay/integration', () => {
  it("extension's final chunk text equals the concatenation of streamed partials", async () => {
    ...
    // Wait for the final chunk. 5 s is well past any localhost
    // round-trip; if we don't see it, dispatch or routing is broken.
    ...
    // full assembled text — the central assertion the contract calls
    // out: extension's final chunk text === concatenation of partials.
    ...
    // there is exactly one client in this harness we additionally
