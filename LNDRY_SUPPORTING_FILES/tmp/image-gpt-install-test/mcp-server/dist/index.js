...
const relayClient_js_1 = require("./relayClient.js");
const workspaceResolver_js_1 = require("./workspaceResolver.js");
const generateImage_js_1 = require("./tools/generateImage.js");
const generateLogo_js_1 = require("./tools/generateLogo.js");
const generateHero_js_1 = require("./tools/generateHero.js");
const generateIconSet_js_1 = require("./tools/generateIconSet.js");
const generateUiMockup_js_1 = require("./tools/generateUiMockup.js");
// ─── Constants ─────────────────────────────────────────────────────────────
...
const TOOLS = [
    {
        name: 'generate_image',
        description: 'Generate a generic image via DALL-E and save it to the workspace.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Generate a coherent icon set (one image per name) sharing a theme.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Generate a UI mockup image of the described component and save it.',
        inputSchema: {
            type: 'object',
            properties: {
 ...
 */
function makeLazyConnect(relayClient, relayUrl) {
    let pending = null;
    return () => {
...
async function main() {
    ...
    const kiroSecret = process.env.KIRO_GPT_MCP_SECRET ?? '';
    const relayUrl = process.env.KIRO_GPT_MCP_RELAY_URL ?? DEFAULT_RELAY_URL;
    if (kiroSecret.length === 0) {
        log('warn', 'mcp_secret_missing', {
            message: 'KIRO_GPT_MCP_SECRET is not set; relay handshake will fail and tool calls will return RELAY_UNREACHABLE',
        });
    }
    ...
    const relayClient = (0, relayClient_js_1.createMcpRelayClient)({
        relayUrl,
        kiroSecret,
        clientVersion: SERVER_VERSION,
    });
    const workspaceResolver = (0, workspaceResolver_js_1.createWorkspaceResolver)();
    const ctx = { relayClient, workspaceResolver };
    ...
    server.setRequestHandler(types_js_1.ListToolsRequestSchema, () => Promise.resolve({ tools: TOOLS }));
    server.setRequestHandler(types_js_1.CallToolRequestSchema, async (req) => {
        ...
        switch (toolName) {
            case 'generate_image':
                result = await (0, generateImage_js_1.generateImage)(args, ctx);
                break;
            case 'generate_logo':
                result = await (0, generateLogo_js_1.generateLogo)(args, ctx);
                break;
            case 'generate_hero':
                result = await (0, generateHero_js_1.generateHero)(args, ctx);
                break;
            case 'generate_icon_set':
                result = await (0, generateIconSet_js_1.generateIconSet)(args, ctx);
                break;
            case 'generate_ui_mockup':
                result = await (0, generateUiMockup_js_1.generateUiMockup)(args, ctx);
                break;
            default:
